...
void sort_kmer_reads(uint8_t *array, uint8_t *offs, uint32_t *rids, kmerhash *hash, int kmer_size, uint32_t rd_len, uint64_t k_range[2], char *rds_file){
	FileReader *fr;
	Sequence *seq;
	kmer_t MER, *mer;
	int i;
	uint32_t j, n, rid;
	uint64_t k, kmer, kmer_mask;
	...
	while((seq_type==1)? (fread_fasta_adv(&seq, fr, FASTA_FLAG_NO_NAME)) : (fread_fastq_adv(&seq, fr, FASTQ_FLAG_NO_NAME | FASTQ_FLAG_NO_QUAL))){
		rid ++;
		if(seq->seq.size < (int)rd_len) continue;
		if(seq->seq.size > (int)rd_len) seq->seq.size = rd_len;
		n ++;
		kmer = 0;
		...
		for(;i<seq->seq.size;i++){
			...
			k = ((uint64_t)mer->idx) * rd_len;
			offs[mer->idx] = i - kmer_size + 1;
			rids[mer->idx] = rid;
			for(j=0;j<rd_len;j++){
				array[(k + j) >> 2] |= (base_bit_table[(int)seq->seq.string[j]] & 0x03) << (((k + j) & 0x03) << 1);
			}
			mer->idx ++;
		}

		reverse_string(&(seq->seq));
		for(j=0;j<rd_len;j++) seq->seq.string[j] = comp_base_table[(int)seq->seq.string[j]];
...
void cluster_reads_by_kmer(char *rds_file, char *kmer_file, uint64_t max_mem, FILE *out){
	...
	kmer_t MER, *mer;
	SeqFileAttr attr;
	uint32_t i, j, s, c, o, kmer_size, rd_len;
	...
	n_limit = ((uint64_t)max_mem * 4) / (rd_len + 16);
	hash = init_kmerhash(1024*1024);
	kmers = init_vec(sizeof(uint64_t), 1024);
	array = (uint8_t*)malloc((((uint64_t)n_limit + MAX_COUNT) * rd_len + 3) / 4);
	offs  = (uint8_t*)malloc(n_limit + MAX_COUNT);
	rids  = (uint32_t*)malloc((n_limit + MAX_COUNT) * sizeof(uint32_t));
	MER.off = MER.idx = MER.count = 0;
	while(load_next_kmer_table(fr, kmers, hash, kmer_size, n_limit, k_range)){
		memset(array, 0, (((uint64_t)n_limit + MAX_COUNT) * rd_len + 3) / 4);
		sort_kmer_reads(array, offs, rids, hash, kmer_size, rd_len, k_range, rds_file);
		for(i=0;i<vec_size(kmers);i++){
			...
			for(j=0;j<c;j++){
				o = (mer->off + j) * rd_len;
				fprintf(out, "%u:", rids[mer->off + j]);
				for(s=0;s<rd_len;s++) fprintf(out, "%c", bit_base_table[(array[(o + s) >> 2] >> (((o + s) & 0x03) << 1)) & 0x03]);
				fprintf(out, ":%u,", offs[mer->off + j]);
			}
			fprintf(out, "\n");
		}
		clear_vec(kmers);
...
int main(int argc, char **argv){
	...
	else max_mem = ((uint64_t)8) * 1024 * 1024 * 1024;
