...
typedef struct {
	uint64_t kmer1, kmer2:54, count:10;
} kmer_t;
...
#define kmer_hashcode(K) MurmurHash64A(&(K), sizeof(kmer_t), 3079ul)
...
void counting(kmerhash *hash, int kmer_size, uint64_t kmer_mask[2], Sequence *seq){
	...
	for(i=0;i<seq->seq.size;i++){
		...
		if(kmer_size <= 32){
			...
			vv[2] = get_rev_seq(v[0], kmer_size);
...
int cmp_kmer_t(const void *e1, const void *e2){
	...
	k1 = (kmer_t*)e1;
	k2 = (kmer_t*)e2;
	if(k1->kmer1 == k2->kmer1){
		if(k1->kmer2 == k2->kmer2) return 0;
...
uint64_t merge_print_kmers(int kmer_size, uint32_t cutoff, uint64_t n_kmers, int is_special, int is_c, FILE *out){
	...
	while(nk < n_kmers){
		...
		for(i=0;i<dump_id;i++){
			if(flags[i] == 0){ n = fread(kmers + i, sizeof(kmer_t), 1, ins[i]); if(n != 1){ flags[i] = 1; kmers[i].kmer2 = 0x3FFFFFFFFFFFFLLU; } else flags[i] = 2;  }
			if(flags[i] == 2){
...
"Usage: [options]\n"
"-l <kmer_size>    Size of kmer (27 ~ 57), default: 51 (bp)\n"
