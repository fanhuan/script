...
for ref in acro_grass Aptero sp_At-9b sp_YR343 strewartii Trachy
  ...
  for sample in ABBM1 ABBM2 ABBM3 ACBM1 ACBM2 ACBM3 ALBM1 ALBM2 ALBM3 ASBM1 ASBM2 ASBM3
    ...
    samtools view -bS ${sample}_$ref.sam.gz | samtools sort > ${sample}_$ref.bam
    samtools mpileup ${sample}_$ref.bam > coverage_${sample}_$ref.txt
    python ~/scripts/coverage2region_general.py coverage_${sample}_$ref.txt > stats_${sample}_$ref.txt
    done
  done
