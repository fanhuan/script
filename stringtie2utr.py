...
def _make_utr_feature(fields, utr_type, utr_start, utr_end, coding_span=None):
    """
    ...
    Returns None if the resulting UTR would be invalid (start > end), or if it
    overlaps the transcript coding span (coding_span=(cds_min, cds_max)). The
    latter guards against a spurious UTR on an INTERNAL exon: when a merged
    StringTie exon boundary disagrees with the BRAKER CDS boundary by a few bp,
    the per-exon logic below would otherwise emit that overhang as a 5'/3' UTR
    sitting inside the CDS. Downstream tools (e.g. SnpEff) then read it as a
    shifted coding start and frameshift the protein. A real UTR lies entirely
    outside the coding span.
    """
    if utr_start > utr_end:
        return None
    if coding_span is not None:
        cds_min, cds_max = coding_span
        if utr_start <= cds_max and utr_end >= cds_min:   # overlaps coding span
            return None
...
def compute_utr_features(tsebra_gtf):
    ...
    - Reject UTR features that fall inside the transcript coding span, so an
      internal exon whose StringTie boundary disagrees with the BRAKER CDS by a
      few bp can no longer produce a 5'/3' UTR inside the CDS (which SnpEff reads
      as a shifted coding start -> frameshifted protein)
    ...
    for transcript_id, features in tsebra_gtf.items():
        ...
        # Coding span of this transcript (min CDS start .. max CDS end). Any
        # inferred UTR must lie entirely outside it; see _make_utr_feature.
        cds_coords = [f.split('\t') for f in features if f.split('\t')[2] == "CDS"]
        if cds_coords:
            cds_min = min(int(c[3]) for c in cds_coords)
            cds_max = max(int(c[4]) for c in cds_coords)
            coding_span = (cds_min, cds_max)
        else:
        ...
        for feature in features:
            ...
            if feature_type == "exon":
                ...
                if overlapping_cds:
                    ...
                    if strand == "+":
                        if start < cds_start:
                            utr = _make_utr_feature(fields, "five_prime_UTR", start, cds_start - 1, coding_span)
                            if utr:
                                utr_features.append(utr)

                        if end > cds_end:
                            utr = _make_utr_feature(fields, "three_prime_UTR", cds_end + 1, end, coding_span)
                            if utr:
                                utr_features.append(utr)

                    elif strand == "-":
                        if end > cds_end:
                            utr = _make_utr_feature(fields, "five_prime_UTR", cds_end + 1, end, coding_span)
                            if utr:
                                utr_features.append(utr)

                        if start < cds_start:
                            utr = _make_utr_feature(fields, "three_prime_UTR", start, cds_start - 1, coding_span)
                            if utr:
                                utr_features.append(utr)
        ...
        for idx, feature in enumerate(features):
            ...
            if "StringTie" in fields[1] and featuretype == "exon":
                # A leftover StringTie exon overlapping the coding span is just a
                # redundant copy of an (internal/terminal) CDS exon — any real UTR
                # overhang was already captured in the first pass. Drop it rather
                # than relabel it as a UTR, which would otherwise place a UTR
                # inside the CDS.
                overlaps_cds = (coding_span is not None and
                                start <= cds_max and end >= cds_min)
