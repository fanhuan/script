...
def aaf_phylokmer(dataDir,samples,k,n,nThreads,memPerThread):
	...
	jobList = []
		...
		command = '{} -l {} -n {} -G {} -o {} -f '.format(kmerCount, k,
...
phylokmer(dataDir,samples,ks,n,options.nThreads,memSize/options.nThreads)
...
for i in range(sn):
	for j in range(i+1,sn):
		...
		#command.append = 'mkdir {}/{}_{}/{}'.format(selection_dir,samples[i], \
													samples[j],samples[i])
		#command.append = 'mkdir {}/{}_{}/{}'.format(selection_dir,samples[i],samples[j],
											 samples[j])
		command.append = '{} -k s -c -d 0 -A A -B A {}.pkdat.gz {}.pkdat.gz | cut -f 1 > test.kmer'.format(filt,samples[i],sample[j])
		command.append = '{} -k test.kmer -o {}/{}_{}/{} -s {}/{}/*' \
		...
		ntotal = []
        for k in [i,j]:
            command = '{} -l {} -n {} -G {} -o {}_temp.pkdat -f FA -i {}/{}_{}/{}.fa' \
				  .format(kmerCount,kl,n,memSize/2, selection_dir,samples[k],
						  samples[k],samples[k])
            status, output = commands.getstatusoutput(command)
	        if status == 0:
		        ntotal.append(float(output.split()[1]))
		#kmer_merge
		command = "{} -k s -c -d '0' -a 'T,M,F' {}_temp.pkdat {}_temp.pkdat | \
                  wc -l".format(filt,samples[i],samples[j])
        status, output = commands.getstatusoutput(command)
